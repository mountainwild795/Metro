...
export const getHouse: RequestHandler = async (req, res, next) => {
  const houseId = req.params.houseId;
  try {
    if (!mongoose.isValidObjectId(houseId)) {
      throw createHttpError(400, "invalid house ID");
    }

    const house = await houseModel.findById(houseId).exec();
    if (!house) {
      throw createHttpError(404, "House not found");
    }
